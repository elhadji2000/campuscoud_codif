...
*/
	...
	$connexion = connexionBD(); Pass_Bd_Cc_2425   U:u893234126_2425	Bd:u893234126_bd_cc_24_25
	*/
	
	 	/* function connexionBD(){ 
        $connexion = mysqli_connect('localhost','u893234126_campuscoud_use','Passcampuscoud_Hostinger_2022', 'u893234126_campuscoud_bd') or die ('Serveur inaccessible. Merci de reessayer plus tard.');
        return $connexion;
	}
	$connexion = connexionBD(); */	
	
/*	function connexionBD(){ 
        $connexion = mysqli_connect('localhost','u893234126_2425','Pass_Bd_Cc_2425', 'u893234126_bd_cc_24_25') or die ('Serveur inaccessible. Merci de reessayer plus tard.');
        return $connexion;
	}
	$connexion = connexionBD();	*/
   ...
   /*  $dbName = "u893234126_BdCc_" . $annee;
    ...
    $connexion = mysqli_connect('localhost', $user, $pw, $dbName)
...
//FONCTION POUR APLLI SECURITE COUD (https://campuscoud/securite/) ///////////////////////////////////////////////   
    ...
    $connexion2 = mysqli_connect('localhost', $user, $pw, $dbName)
        or die('Serveur inaccessible. Merci de reessayer plus tard.');
...
function getEtudiantByLit($lit, $paie, $connexion) {
    $sql = "
        ...
        ORDER BY 
            FIELD(lg.statut, 'Attributaire', 'Suppleant(e)', 'Hébergé');
    ";
...
curl_setopt_array($curl, array(
  CURLOPT_URL => 'http://173.230.133.221/wsSendSMS',
...
function getDonneesEtudiant_2($numero_carte)
{
    ...
    try {
        ...
        if (isset($data[0]) && is_object($data[0])) {
            ...
            $result['sexe'] = $data[0]->sexe ?? null; if($result['sexe']=='M'){$result['sexe']='G';}
...
/********************************************************************************** 
Fonction d'affichage de la liste des etablissements, gestion des quota  des FAC figurant dans la table Delai
 ********************************************************************************* */
function getAllEtablissement_dl()
{
    global $connexion;
    $requeteListeEtablissement = "SELECT DISTINCT (faculte) as etablissement FROM `codif_delai` order by etablissement asc";
    $resultatRequeteEtablissement = mysqli_query($connexion, $requeteListeEtablissement);
    return $resultatRequeteEtablissement;
}
...
function getPaiementWithDateInterval($date_debut, $date_fin,$username)
{
	...
	WHERE pc.dateTime_paie >= '$date_debut' AND pc.dateTime_paie <= '$date_fin' and pc.username_user='$username' order by pc.id_paie desc";
    //$result = mysqli_query($connexion, $sql);
...
function getLitParChambre($link, $chambre)
{
    ...
    ORDER BY 
        FIELD(statut, 'Attributaire', 'Suppleant(e)', 'Hébergé'),
        lit ASC
