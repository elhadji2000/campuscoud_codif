...
<head>
    ...
    /* Pour aligner parfaitement les colonnes */
    .row>div {
        display: flex;
        align-items: center;
    }
    ...
    <div class="container">
        ...
        <form action="traitement_add_delai.php" method="POST">
            <div class="row g-3 align-items-center">
                ...
                <div class="col-md-4">
                    <select name="nature" class="form-select uniform-field" required>
                        <option selected disabled>CHOISIR NATURE</option>
                        <option value="depart">Depart</option>
                        <option value="choix">Forclusion Choix Lit</option>
                ...
                <div class="col-md-4">
                    <input type="date" name="date" class="form-control uniform-field" required>
                </div>

                <div class="col-md-4">
                    <select name="faculte[]" class="selectpicker w-100" multiple data-live-search="true"
                        title="CHOISIR FACULTE" required>

                        <?php
                $dataEtablissement = getAllEtablissement();
                while ($rowNiv = mysqli_fetch_array($dataEtablissement)) { ?>
                        <option value="<?= $rowNiv['etablissement']; ?>/SOCIAL">
                            <?= $rowNiv['etablissement']; ?>/SOCIAL
                        </option>
                        <?php } ?>
                    </select>
                </div>
