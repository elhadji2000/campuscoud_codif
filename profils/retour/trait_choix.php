...
foreach ($result2 as $row) {
    if (!empty($row['id_suppleant'])) {

        $suppleants[] = [
            'id_etu' => $row['id_suppleant'],
            'num_etu' => $row['num_suppleant'], // adapte si tu as num_etu
            'classe' => $row['niveau_suppleant'],
            'sexe' => $row['sexe'],
            'rang' => $row['rang_suppleant'] ?? null,
            'id_titulaire' => $row['id_etu']
        ];
        ...
        $stmt = $connexion->prepare("
            ...
            ");
            $stmt->bind_param('is', $row['id_etu'], $_SESSION['username']);
            $stmt->execute();
            $stmt->close();
    }
}
...
if (empty($lits) || empty($suppleants)) {
    $_SESSION['message'] = 'Aucune affectation possible (pas de suppleant(e) // Attributaire Indiv Forclo(s) ! ';
...
try {
    ...
    foreach ($lits as $keyLit => $lit) {
        ...
        foreach ($suppleants as $keySup => $suppleant) {
            ...
            $idTitulaire = $suppleant['id_titulaire'];
