...
function getOrCreateStudentId($connexion, $num_etu, $niveauFormation, $moyenneFixee, $litActuel)
{
    ...
    if ($id) {
        ...
        if ($dataStatutStudent['statut'] == 'Attributaire') {
            ...
            // S'il est attributaire ailleurs → erreur
            if (!$dejaDansCeLit && isAffecteDansAutreLit($id, $litActuel)) {
                error("L’étudiant ($num_etu) est déjà attributaire dans un autre lit.");
            }

            // Sinon → OK (il est déjà dans ce lit)
        }
        ...
        if ($dataStatutStudent['statut'] == 'Suppleant(e)') {
            ...
            if (!$dejaDansCeLit && isAffecteDansAutreLit($id, $litActuel)) {
                error("L’étudiant ($num_etu) est déjà suppléant dans un autre lit.");
            }
        }
