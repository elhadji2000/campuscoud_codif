...
if ($type === 'titulaire') {
    if (!$idLocal) {
        echo json_encode([
            'success' => false,
            'message' => "Letudiant doit obligatoirement exister dans la base locale!"
        ]);
        exit;
    }
 ...
 if (stripos($etudiant_2['etablissement'], 'social') === false) {
    echo json_encode([
        'success' => false,
        'message' => 'Attention, cet etudiant na pas une codification sociale!!!'
    ]);
    exit;
}
*/
...
$anneeCourante = intval(substr($_SESSION['annee'], -4));
